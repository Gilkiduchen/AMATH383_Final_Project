...
For the current generated Wilson-Cowan parameter set, the numerical sweep finds one equilibrium for each value of `P`. The equilibrium loses local stability over a finite `P` interval, and the eigenvalues near the stability crossings are complex. The tracked equilibrium is unstable over approximately `P = 1.64` to `P = 2.80` on the chosen sweep grid, with actual crossings lying between neighboring grid points. Therefore the current result should be described as a Hopf-like loss of stability that can support oscillatory seizure-like behavior, not as a saddle-node or hysteresis transition.
...
Preictal labels are record-local in the current implementation; windows in neighboring EDF records are not assigned time-to-next-seizure labels across file boundaries. This limits preictal-transition claims and keeps the empirical classifier framed as ictal-window detection.
