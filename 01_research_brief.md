...
Use a low-dimensional neural population ODE to model loss of local stability and seizure-like oscillatory activity, then compare the model's qualitative implications with features and ictal-window classifier outputs from the existing CHB-MIT EEG pipeline. Use the Consistency Index (CI) and RFECV idea from the uploaded Environmental Research paper to test whether feature explanations are stable enough to support interpretation.

## Course Fit

From the AMATH 383 final project guideline file, the project should:
...
Wilson-Cowan models excitatory and inhibitory population activity with coupled nonlinear differential equations. Epilepsy-specific Wilson-Cowan work shows that bifurcation analysis can produce seizure-like activity regimes. In the current generated project outputs, the selected parameters show one equilibrium losing local stability over a finite interval of the excitability parameter, which should be described as Hopf-like rather than saddle-node/hysteretic.
...
6. The Water Research X ammonia-nitrogen paper is less central but useful as a modeling precedent.

The ammonia-nitrogen paper shows a practical ML early-warning workflow on noisy real-world process data. It is useful background for data-driven monitoring, but the current CHB-MIT classifier is ictal-window detection rather than seizure prediction.

## Proposed Research Questions

1. Can a two-variable excitatory-inhibitory ODE reproduce a qualitative transition from stable normal activity to seizure-like oscillatory activity through loss of local stability?
...
- Use CHB-MIT windows as empirical context for ictal separability and feature trajectories.
